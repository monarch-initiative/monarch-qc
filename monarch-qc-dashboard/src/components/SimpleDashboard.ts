...
for (const name of names) {
    ...
    const a_total = a.get(name) ?? 0
    const b_total = b.get(name) ?? 0
    const diff = visualDiff(a_total, b_total)
    edge_diff.set(name, diff)
}
if (a.get("Total Number") !== undefined && b.get("Total Number") !== undefined) {
    const diff = visualDiff(a.get("Total Number")!, b.get("Total Number")!)
    edge_diff.set("Total Number", diff)
}
return edge_diff
}

export function visualDiff(a: number, b: number): string {
/**
